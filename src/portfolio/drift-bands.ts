 ...
 */
import { discountEligibleFrom, type IsoDate } from '../tax/au-dates.js';
 ...
 * discount — the tax module's rule (buy date + 1 year + 1 day), so the guard
 * and the tax report can never disagree about a boundary.
 */
export function discountDate(buyDate: string): string {
  return discountEligibleFrom(buyDate as IsoDate);
}
